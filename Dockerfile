...
RUN (crontab -l ; echo "0 23 * * * /db-backups.sh >> /var/log/cron.log 2>&1") | crontab
RUN (crontab -l ; echo "0 23 */15 * * /file-backups.sh >> /var/log/cron.log 2>&1") | crontab
RUN (crontab -l ; echo "0 23 */7 * * /clean.sh >> /var/log/cron.log 2>&1") | crontab
